...
boolean CommandisOk=false;
  ...
  //  e.printStackTrace();  
  //}
  //final JFileChooser fc = new JFileChooser("/home/geraldo/Dropbox/Projetos/Anem");
  final JFileChooser fc = new JFileChooser("C:/Users/geraldo/My Dropbox/Projetos/Iniciacao Cientifica/Desenvolvimento em VHDL/Software de teste");
  ...
  if((portChosen == true) && (abortrun==false)){
    ...
    if ((index < lines.length) && (index > -1)) {
      ...
      if (pieces.length == 2) {
        ...
        subword = byte((word & 65280) >> 8); // msb do endereco
        ...
        subword = byte(word & 255); // lsb do endereco
        ...
        subword = byte((word & 65280) >> 8); // msb da instrucao
        ...
        subword = byte(word & 255); // lsb da instrucao
    ...
    for(i=0; i<Serial.list().length; i++){
      text("[" + i + "] " + Serial.list()[i], 10, topofpage+(3+i*1.5)*linecomp);
    }
  }
}
...
void keyPressed(){ //se uma tecla é pressionada
  tecla = key;
  ...
  if(portChosen == false){
    ...
    else
    {
      ...
      println(COMPort);
      serPorta = new Serial(this, Serial.list()[COMPort], 9600, 'N', 8, 1.0);
      portChosen = true;
      abortrun=false;
    }
  }
...
void MsgdeErro(){
  ...
  textFont(fontA, 14);
  text("1. Anem16 está no modo PROG?",width/2,topofpage+8.5*linecomp);
  text("2. Cabo está bem conectado?",width/2,topofpage+10*linecomp);
  text("3. Porta serial foi escolhida corretamente?",width/2,topofpage+11.5*linecomp);
  textFont(fontA, 12);
  text("Após corrigido o problema, reinicie este programador teclando 'r'.",width/2,topofpage+14*linecomp);
  text("É aconselhável também reiniciar o Anem16.",width/2,topofpage+15.5*linecomp);
  textFont(fontA, 16);
  textAlign(LEFT);
  abortrun=true;
}
